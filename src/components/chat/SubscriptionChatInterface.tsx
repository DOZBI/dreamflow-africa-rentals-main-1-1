...
interface SubscriptionChatInterfaceProps {
  conversationId?: string;
  onBack?: () => void;
}

export const SubscriptionChatInterface = ({ conversationId, onBack }: SubscriptionChatInterfaceProps) => {
  ...
  const setupConversationsSubscription = () => {
    ...
    conversationsChannelRef.current = supabase
      .channel('subscription_user_conversations')
      .on(
        'postgres_changes',
        {
          event: '*',
  ...
  const setupMessagesSubscription = (convId: string) => {
    ...
    messagesChannelRef.current = supabase
      .channel(`subscription_messages_${convId}`)
      .on(
        'postgres_changes',
        { 
          event: 'INSERT', 
  ...
  const addNewMessage = async (newMessageData: any) => {
    const { data: completeMessage, error } = await supabase
      ...
      .select(`
        ...
        message_type,
        audio_url,
        sender_profile:subscription_profiles!subscription_messages_sender_code_fkey (
          full_name,
          avatar_url
        )
      `)
  ...
  const sendMessage = useCallback(async (messageContent: string) => {
    if (!code || !selectedConversation) return;
  ...
  }, [code, selectedConversation, toast]);
          ...
          {isLoading ? (
            ...
            <>
              {messages.map((message, index) => {
                const isCurrentUser = message.sender_code === code;
